...
test("setQuestionLike sends authenticated intent and parses promotion state", async () => {
  ...
  const result = await setQuestionLike({ getToken: async () => "token", questionId: "q-1", liked: true, fetchImpl: async (_input, init) => {
    assert.equal((init?.headers as Record<string, string>).Authorization, "Bearer token");
    assert.equal(init?.body, JSON.stringify({ questionId: "q-1", liked: true }));
