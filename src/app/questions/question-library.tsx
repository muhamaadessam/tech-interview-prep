...
export function QuestionLibrary({ questions, topics, locale = "ar" }: { questions: SearchableQuestion[]; topics: LibraryTopic[]; locale?: Locale }) {
  ...
  const displayQuestions = useMemo<DisplayQuestion[]>(() => {
    ...
    if (filters.scope === "community") return community.filter((question) => question.visibility === "community" || Boolean(question.promotedAt));
  ...
  const matchingQuestions = scoped ? filterQuestions(displayQuestions, filters, saved, scoped.topics) : [];
  ...
  async function toggleLike(question: DisplayQuestion) {
    ...
    } finally { setLikeBusy((current) => ({ ...current, [question.id]: false })); }
  }
  ...
  return <>
    ...
    {phase !== "ready" || invalidTrack || !activeTrack ? null : scoped?.invalidTopic ? <ActiveTrackRecovery locale={locale} invalidTopic /> : !scoped?.topics.length ? <div className="empty-state"><h2>{copy.emptyTrackTitle}</h2><p>{copy.emptyTrackDescription}</p></div> : <>
      <div className="scope-tabs" role="tablist" aria-label={copy.questions}><button type="button" className={filters.scope === "public" ? "active" : ""} onClick={() => updateFilters({ scope: "public" })}>{copy.publicScope}</button><button type="button" className={filters.scope === "community" ? "active" : ""} onClick={() => updateFilters({ scope: "community" })}>{copy.communityScope}</button></div>
      <form className="library-filters" onSubmit={(event) => event.preventDefault()}>
        ...
        <label>{copy.topic}<select value={filters.topic} onChange={(event) => updateFilters({ topic: event.target.value })}><option value="">{copy.allTopics}</option>{scoped.topics.map((topic) => <option key={topic.id} value={topic.slug}>{topic.name}</option>)}</select></label>
