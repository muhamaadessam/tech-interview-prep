...
type DatabaseConfig = { url: string; key: string };

function apiUrl(path: string, config: DatabaseConfig): string {
  return `${config.url}/rest/v1/${path}`;
}

async function read<T>(path: string, fetchImpl: typeof fetch, config: DatabaseConfig): Promise<T> {
  const response = await fetchImpl(apiUrl(path, config), { headers: { apikey: config.key, Accept: "application/json" } });
...
export function createDatabaseQuestionReader({ url, key, fetchImpl = fetch }: { url: string; key: string; fetchImpl?: typeof fetch }) {
  return (slug: string, locale: Locale) => loadDatabaseQuestion(slug, locale, fetchImpl, { url: url.replace(/\/$/, ""), key });
}

export async function loadDatabaseQuestion(slug: string, locale: Locale, fetchImpl: typeof fetch = fetch, config?: DatabaseConfig): Promise<DatabaseQuestion> {
  ...
  if (nodeUrl && !config) {
    ...
    catch (error) { if ((error as { status?: number }).status === 404) throw new DatabaseQuestionNotFound(); throw error; }
  }
  const resolvedConfig = config ?? (supabaseUrl && supabaseKey ? { url: supabaseUrl, key: supabaseKey } : null);
  if (!resolvedConfig) throw new Error("database_config_missing");
  const encodedSlug = encodeURIComponent(slug);
  const questions = await read<QuestionRow[]>(`interview_questions?select=id,slug,track_id,difficulty,published_revision_id&slug=eq.${encodedSlug}&published_revision_id=not.is.null&or=(visibility.eq.public,community_unpublished_at.is.null)&limit=1`, fetchImpl, resolvedConfig);
  ...
  const revisions = await read<RevisionRow[]>(`question_revisions?select=id,question_id,reviewed_at&id=eq.${revisionId}&question_id=eq.${encodeURIComponent(row.id)}&status=eq.published&limit=1`, fetchImpl, resolvedConfig);
  const revision = revisions[0];
  if (!revision || typeof revision.reviewed_at !== "string") throw new Error("database_payload_invalid");
  const locales = await read<LocaleRow[]>(`question_revision_locales?select=locale,question,short_answer,explanation,code_example,common_mistakes,follow_up_questions,sources&revision_id=eq.${revisionId}`, fetchImpl, resolvedConfig);
  ...
  const followUps = await read<FollowUpRow[]>(`question_follow_ups?select=target_question_id,position&source_revision_id=eq.${revisionId}&order=position.asc`, fetchImpl, resolvedConfig);
  ...
  if (targetIds.length) {
    ...
    const targetQuestions = await read<TargetQuestionRow[]>(`interview_questions?select=id,slug,track_id,published_revision_id&id=in.(${encodedTargets})&track_id=eq.${encodeURIComponent(row.track_id)}&published_revision_id=not.is.null`, fetchImpl, resolvedConfig);
    ...
    const targetLocales = targetRevisionIds.length ? await read<TargetRevisionLocaleRow[]>(`question_revision_locales?select=revision_id,locale,question&revision_id=in.(${targetRevisionIds.map(encodeURIComponent).join(",")})&locale=in.(ar,en)`, fetchImpl, resolvedConfig) : [];
  ...
  const topics = await read<TopicRow[]>(`question_topics?select=topic_id&question_id=eq.${encodeURIComponent(row.id)}`, fetchImpl, resolvedConfig);
  ...
  const topicLocales = await read<TopicLocaleRow[]>(`topic_locales?select=topic_id,name&topic_id=in.(${topicFilter})&locale=eq.${locale}`, fetchImpl, resolvedConfig);
