...
test("Track Preference saves delegate identity to Clerk-backed RPC", async () => {
  ...
  await store.savePreferences("user_123", ["flutter", "flutter"], "flutter", "clerk-token");

  assert.equal(request?.url, "https://db.example/rest/v1/rpc/set_track_preferences");
  ...
  await assert.rejects(store.savePreferences("user_123", ["flutter"], "flutter"), /track_preferences_token_required/);
});
...
test("Track Preference RPC validation keeps stable client errors", async () => {
  ...
  await assert.rejects(store.savePreferences("user_123", ["flutter"], "flutter", "clerk-token"), (error: unknown) => error instanceof TrackStoreError && error.code === "tracks_required" && error.status === 400);
});
...
test("Track Preference RPC outages preserve retryable status", async () => {
  ...
  await assert.rejects(store.savePreferences("user_123", ["flutter"], "flutter", "clerk-token"), (error: unknown) => error instanceof TrackStoreError && error.status === 503);
});
