...
async function db(path: string, key: string, init: RequestInit = {}, fetchImpl: FetchLike = fetch): Promise<Response> {
  const result = await fetchUpstream(fetchImpl, `${process.env.SUPABASE_URL?.replace(/\/$/, "")}${path}`, { ...init, headers: { apikey: key, Authorization: `Bearer ${key}`, "Content-Type": "application/json", ...(init.headers ?? {}) } });
  if (!result.ok) throw new Error("database_error");
  return result;
}

async function isModerator(userId: string, key: string, fetchImpl: FetchLike = fetch): Promise<boolean> {
  const rows = await (await db(`/rest/v1/account_roles?select=role,suspended&user_id=eq.${encodeURIComponent(userId)}&limit=1`, key, {}, fetchImpl)).json() as Array<{ role?: string; suspended?: boolean }>;
...
async function githubToken(fetchImpl: FetchLike): Promise<string> {
  ...
  const result = await fetchUpstream(fetchImpl, `https://api.github.com/app/installations/${encodeURIComponent(installationId)}/access_tokens`, { method: "POST", headers: { Accept: "application/vnd.github+json", Authorization: `Bearer ${header}.${payload}.${base64Url(new Uint8Array(signature))}`, "X-GitHub-Api-Version": "2022-11-28", "User-Agent": "tech-interview-prep" } });
...
async function postAdvisoryComment(issueNumber: number, body: string, key: { submissionId: string; revisionNumber: number }, fetchImpl: FetchLike): Promise<number> {
  ...
  const comments = await fetchUpstream(fetchImpl, `https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repository)}/issues/${issueNumber}/comments?per_page=100`, { headers });
  ...
  const created = await fetchUpstream(fetchImpl, `https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repository)}/issues/${issueNumber}/comments`, { method: "POST", headers: { ...headers, "Content-Type": "application/json" }, body: JSON.stringify({ body }) });
...
async function openAiReview(envelope: ReturnType<typeof buildAdvisoryEnvelope>, fetchImpl: FetchLike): Promise<ReturnType<typeof validateAdvisoryResult>> {
...
export async function handleAdvisory(request: Request, fetchImpl: FetchLike = fetch): Promise<Response> {
  ...
  try {
    const { key } = config();
    if (!(await isModerator(userId, key, fetchImpl))) return response({ error: "moderator_required" }, 403);
    ...
    const submissions = await (await query(`/rest/v1/submissions?select=id,status,track_id,topic_ids,difficulty,github_issue_number&id=eq.${encodeURIComponent(submissionId)}&limit=1`, key)).json() as DbRow[];
    ...
    const revisions = await (await query(`/rest/v1/submission_revisions?select=revision_number,track_id,topic_ids,difficulty,payload&submission_id=eq.${encodeURIComponent(submissionId)}&revision_number=eq.${revisionNumber}&limit=1`, key)).json() as DbRow[];
    ...
    const existingRows = await (await query(`/rest/v1/submission_advisories?select=id,status,result,github_comment_id&submission_id=eq.${encodeURIComponent(submissionId)}&revision_number=eq.${revisionNumber}&prompt_version=eq.${advisoryPromptVersion}&provider=eq.${advisoryProvider}&model=eq.${advisoryModel}&limit=1`, key)).json() as AdvisoryRow[];
    ...
    if (!existing) {
      try {
        existing = (await (await query("/rest/v1/submission_advisories", key, { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify([{ submission_id: submissionId, revision_number: revisionNumber, status: "running", provider: advisoryProvider, model: advisoryModel, prompt_version: advisoryPromptVersion }]) })).json() as AdvisoryRow[])[0];
        claimed = true;
      } catch {
        existing = (await (await query(`/rest/v1/submission_advisories?select=id,status,result,github_comment_id&submission_id=eq.${encodeURIComponent(submissionId)}&revision_number=eq.${revisionNumber}&prompt_version=eq.${advisoryPromptVersion}&provider=eq.${advisoryProvider}&model=eq.${advisoryModel}&limit=1`, key)).json() as AdvisoryRow[])[0];
      }
    } else {
      await query(`/rest/v1/submission_advisories?id=eq.${encodeURIComponent(existing.id)}`, key, { method: "PATCH", body: JSON.stringify({ status: "running", last_error: null }) });
    }
    ...
    try {
      if (!result) result = await openAiReview(buildAdvisoryEnvelope({ submissionId, trackId: String(revision.track_id ?? submission.track_id), topicIds: revision.topic_ids, difficulty: revision.difficulty, payload: revision.payload }), fetchImpl);
      if (!result) throw new Error("openai_schema_invalid");
      await query(`/rest/v1/submission_advisories?id=eq.${encodeURIComponent(existing.id)}`, key, { method: "PATCH", body: JSON.stringify({ status: "comment_pending", result, last_error: null }) });
      const commentId = await postAdvisoryComment(Number(submission.github_issue_number), advisoryComment(result, { submissionId, revisionNumber }), { submissionId, revisionNumber }, fetchImpl);
      await query(`/rest/v1/submission_advisories?id=eq.${encodeURIComponent(existing.id)}`, key, { method: "PATCH", body: JSON.stringify({ status: "completed", github_comment_id: commentId, last_error: null }) });
      return response({ status: "completed", commentId });
    } catch (error) {
      const code = error instanceof Error ? error.message.slice(0, 120) : "advisory_failed";
      await query(`/rest/v1/submission_advisories?id=eq.${encodeURIComponent(existing.id)}`, key, { method: "PATCH", body: JSON.stringify({ status: result ? "comment_pending" : "failed", result, last_error: code }) }).catch(() => undefined);
